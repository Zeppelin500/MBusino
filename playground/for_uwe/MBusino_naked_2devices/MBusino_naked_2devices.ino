    ...
    for(uint8_t k=0;k<MBUS_SLAVES;k++){
      ...
      */
      if (mbus_good_frame) {
        ...
        client.publish(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/error", String(payload.getError()));  // kann auskommentiert werden wenn es läuft
        client.publish(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/jsonstring", String(jsonstring));
        ...
        for (uint8_t i=0; i<fields; i++) {
          ...
          client.publish(String(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/"+String(i+1)+"_vs_"+String(name)), valueString); // send the value if a ascii value is aviable (variable length)
          client.publish(String(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/"+String(i+1)+"_"+String(name)), String(value,3).c_str()); // send the value if a real value is aviable (standard)
          client.publish(String(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/"+String(i+1)+"_"+String(name)+"_unit"), units);
          //or only one message
          //client.publish(String(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/"+String(i+1)+"_"+String(name)+"_in_"+String(units)), String(value,3).c_str());
          ...
          if (i == 3){  // Sensostar Bugfix --> comment it out if you use not a Sensostar
            ...
            client.publish(MBUSINO_NAME"_Slave"+String(k+1)+"/MBus/4_power_calc", String(calc_power).c_str());           
          }  
