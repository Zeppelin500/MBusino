...
const char adValueMbus[] PROGMEM = R"rawliteral({"unique_id":"%s_%u_%s","object_id":"%s_%u_%s","state_topic":"%s/MBus/%u_%s","name":"%u_%s","value_template":"{{value_json if value_json is defined else 0}}","unit_of_meas":"%s","state_class":"%s","device":{"ids": ["%s"],"name":"%s","mdl":"%s"},"device_class":"%s","availability_mode":"all"})rawliteral";
...
const char* getUnitOfMeassuremetn(const char* getunit){
  if (!strcmp(getunit,"C")) {
      ...
      return "K";
  }else if (!strcmp(getunit,"W")) {
      return "W";
  }else if(!strcmp(getunit,"Wh")) {
      return "Wh";
...
void haHandoverMbus(uint8_t haCounter,const char* haName, const char* units){ // haCounter is the "i+1" at the for() in main
  ...
  strcpy(adVariables.haUnits,"none");
  strcpy(adVariables.stateClass,"total");
  strcpy(adVariables.deviceClass,"none");  
  }
