...
#define MBUSINO_VERSION "0.9.10d"
...
//--------------debug-------------------------------------------
uint16_t zaehlerVor68 = 0;
byte komplettesTelegramm[300] = {0};
uint8_t fehlercode = 0;
/*
1 = max j erreicht
2 = bid > len_pdata
3 =  3rd byte is also length, check that its the same as 2nd byte
4 = 4th byte is the start byte again
5 = Validate checksum
6 = i.o.
*/
...
void loop() {
  ...
  if((millis()-timerDebug) >10000){
    ...
    client.publish(String(String(userData.mbusinoName) + "/settings/ethIP").c_str(), String(Ethernet.localIP().toString()).c_str());
...
////////// M- Bus ###############################################
  ...
  if(millis() - timerMbus > 2000 && mbusLoopStatus == 1){ // Receive and decode M-Bus Records
    mbusLoopStatus = 2;
    ...
    //------------------ only for debug, you will recieve the whole M-Bus telegram bytewise in HEX for analysis -----------------
      ...
      for (int i=0;i<290;i++){
        char buffer[2];  
        sprintf(buffer,"%02X",komplettesTelegramm[i]); 
        telegram2[i*2] = buffer[0];
        telegram2[(i*2)+1] = buffer[1];         
      } 
   
      client.publish(String(String(userData.mbusinoName) +"/debug/telegram").c_str(), telegram);    
      client.publish(String(String(userData.mbusinoName) +"/debug/komplettesTelegramm").c_str(), telegram2);
      client.publish(String(String(userData.mbusinoName) +"/debug/zaehlerVor68").c_str(),String(zaehlerVor68).c_str());
      client.publish(String(String(userData.mbusinoName) +"/debug/fehlercode").c_str(), String(fehlercode).c_str());    
      
      for (int i=0;i<600;i++){
        komplettesTelegramm[i] = 0;
      }  
      zaehlerVor68 = 0;
      fehlercode = 0;                                                                              //|
      //--------------------------------------------------------------------------------------------------------------------------    
  ...
  if(millis() - timerMbus > 3000 && mbusLoopStatus == 2){  // Send decoded M-Bus secords via MQTT
    mbusLoopStatus = 0;
    JsonDocument root;
